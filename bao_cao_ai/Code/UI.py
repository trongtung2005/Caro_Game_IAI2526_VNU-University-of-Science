...
class CaroUI:
    ...
    def hint_calculation_worker(self):
        ...
        print("AI is scanning for the optimal move...")
        with self.logic.lock:
        ...
        if move:
            ...
            print(f"Hint: You should play at {chr(move[1] + ord('A'))}{move[0]+1}!")
    ...
    def bot_calculation_worker(self, side, depth):
        ...
        if move and not self.logic.game_over and not self.is_paused:
            ...
            if success:
                ...
                bot_name = "Bot O (AI 1)" if side == -1 else "Bot X (AI 2)" if self.game_mode == "EvE" else "Bot (AI)"
                print(f"-> {bot_name} replies: {move_str}")
                if is_win: print(f"\n[GAME OVER] {bot_name.upper()} WINS!")
        self.logic.bot_thinking = False
    ...
    def draw_ui(self, screen, font):
        ...
        if self.game_mode in ["PvE", "PvP"]:
            ...
            txt_surf = btn_font.render("RESTART", True, (255,255,255))
        ...
        elif self.game_mode == "PvP": who_is_playing = "PLAYER 1" if self.logic.current_turn == -1 else "PLAYER 2"
        else: who_is_playing = "BOT" if self.logic.current_turn == self.bot_side else "YOU"
            
        if getattr(self.logic, 'game_over', False): status_text = "GAME OVER"
        else: status_text = f"TURN: {who_is_playing} ({turn_char})"

        info_font = pygame.font.SysFont("tahoma", 13, bold=True)
        step_text = f"TOTAL MOVES: {step_count}"
        ...
        time_total_str = f"TOTAL TIME: {tm:02d}:{ts:02d}"
        time_text_o = f"TIME O: {m_o:02d}:{s_o:02d}"
        time_text_x = f"TIME X: {m_x:02d}:{s_x:02d}"
    ...
    def show_mode_selection(self, screen, font):
        ...
        while running:
            ...
            screen.blit(title_font.render("SELECT MODE", True, (0, 255, 255)), (screen.get_width() // 2 - 140, 50))

            pygame.draw.rect(screen, (155, 89, 182), btn_pvp, border_radius=10)
            txt_pvp = sub_font.render("2 PLAYERS (PvP)", True, (255,255,255))
            ...
            txt_pve = sub_font.render("1 PLAYER (VS AI)", True, (255,255,255))
            ...
            txt_eve = sub_font.render("AI VS AI (AUTO)", True, (255,255,255))
            screen.blit(txt_eve, (btn_eve.centerx - txt_eve.get_width()//2, btn_eve.centery - txt_eve.get_height()//2))

            txt_music = vol_font.render(f"MUSIC: {int(self.vol_music*100):02d}%", True, (0, 255, 255))
            txt_sfx = vol_font.render(f"SFX: {int(self.vol_sfx*100):02d}%", True, (0, 255, 255))
    ...
    def show_menu(self, screen, font):
        ...
        while menu_running:
            ...
            txt_back = font.render("<- BACK", True, (255,255,255))
            screen.blit(txt_back, (btn_back.centerx - txt_back.get_width()//2, btn_back.centery - txt_back.get_height()//2))

            screen.blit(title_font.render("GAME SETTINGS", True, (0, 255, 255)), (screen.get_width() // 2 - 140, 30))

            screen.blit(sub_font.render("1. Board Size:", True, self.COLOR_TEXT), (60, 80))
            ...
            if self.game_mode == "PvE":
                screen.blit(sub_font.render("2. AI Level (Difficulty):", True, self.COLOR_TEXT), (60, 190))
                ...
                screen.blit(font.render("EASY", True, (255,255,255)), (btn_easy.centerx-10, btn_easy.centery-10))
                screen.blit(font.render("MEDIUM", True, (255,255,255)), (btn_medium.centerx-15, btn_medium.centery-10))
                screen.blit(font.render("HARD", True, (255,255,255)), (btn_hard.centerx-12, btn_hard.centery-10))

                screen.blit(sub_font.render("3. First Move (Always O):", True, self.COLOR_TEXT), (60, 300))
                ...
                screen.blit(font.render("PLAYER (O)", True, (0,0,0) if self.player_side == -1 else (255,255,255)), (btn_first_player.centerx-35, btn_first_player.centery-10))
                screen.blit(font.render("BOT (O)", True, (0,0,0) if self.bot_side == -1 else (255,255,255)), (btn_first_bot.centerx-30, btn_first_bot.centery-10))
            
            elif self.game_mode == "EvE":
                screen.blit(sub_font.render("2. AI 1 Level (O - First):", True, self.COLOR_O), (60, 190))
                ...
                screen.blit(sub_font.render("3. AI 2 Level (X - Second):", True, self.COLOR_X), (60, 300))
                ...
                screen.blit(font.render("EASY", True, (255,255,255)), (btn_ai2_easy.centerx-10, btn_ai2_easy.centery-10))
                screen.blit(font.render("MEDIUM", True, (255,255,255)), (btn_ai2_medium.centerx-15, btn_ai2_medium.centery-10))
                screen.blit(font.render("HARD", True, (255,255,255)), (btn_ai2_hard.centerx-12, btn_ai2_hard.centery-10))
            
            elif self.game_mode == "PvP":
                screen.blit(sub_font.render("2. PvP Mode: Player vs Player", True, (155, 89, 182)), (60, 220))
                screen.blit(font.render("Player 1: O (First), Player 2: X (Second)", True, (255, 255, 255)), (60, 260))

            pygame.draw.rect(screen, (0, 255, 255), btn_start, border_radius=10)
            text_start = sub_font.render("START GAME", True, (0, 0, 0))
    ...
    def run(self):
        ...
        while True:
            ...
            if self.game_mode == "PvE": print(f"\n=== MATCH: Player vs AI (Level {self.logic.bot_depth}) ===")
            elif self.game_mode == "PvP": print(f"\n=== MATCH: Player vs Player (PvP) ===")
            else: print(f"\n=== MATCH: AI 1 (Level {self.ai_1_depth}) vs AI 2 (Level {self.ai_2_depth}) ===")
                ...
                if self.logic.game_over or is_full:
                    ...
                    if not self.game_over_processed:
                        ...
                        if win_line:
                            ...
                            # If no win_line but board is full -> DRAW
                    ...
                    # HANDLE RESULT NOTIFICATION TEXT
                    if win_line is None:
                        msg, color = ("DRAW GAME!", (255, 255, 0))
                    elif self.game_mode == "PvE":
                        msg, color = ("YOU WIN!", (57, 255, 20)) if self.logic.current_turn == self.player_side else ("YOU LOSE!", (255, 0, 128))
                    elif self.game_mode == "PvP":
                        winner_name = "PLAYER 1 (O)" if self.logic.current_turn == -1 else "PLAYER 2 (X)"
                        msg, color = (f"{winner_name} WINS!", (0, 255, 255))
                    else:
                    ...
                    sub_text = pygame.font.SysFont("tahoma", 14).render("(Click anywhere to return to Menu)", True, (255, 255, 255))
